...
class Signal(log.Logger):
    ...
    def __init__(
        ...
        try:
            ...
            self.spectral_units = self._normalize_units(u.pix)
        ...
        if data_units is not None:
            self.data_units = self._normalize_units(u.Unit(data_units))
        elif hasattr(data, "unit"):
            self.data_units = self._normalize_units(data.unit)
        else:
...
class Radiance(Signal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(data, "unit") and self._normalize_units(
            data.unit
        ) != self._normalize_units(u.W / u.m**2 / u.um / u.sr):
            try:
...
class CountsPerSecond(Signal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(data, "unit") and self._normalize_units(
            data.unit
        ) != self._normalize_units(u.ct / u.s):
            try:
